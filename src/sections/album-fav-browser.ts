...
export class AlbumFavBrowser extends FavBrowserBase {

  /** Array of items to display in the media list. */
  protected override mediaList!: Array<IAlbum> | undefined;
  ...
  constructor() {
    ...
    this.filterCriteriaPlaceholder = "filter by album name";
        ...
        <div id="mediaBrowserContentElement" class="media-browser-content">
          ...
          ${(() => {
            ...
            // if actions are visbile, then render the actions display.
              ...
              return html`<spc-album-actions class="media-browser-actions" .store=${this.store} .mediaItem=${this.mediaItem}></spc-album-actions>`;
            }
          })()}  
        </div>
      </div>
  ...
  protected override updateMediaList(player: MediaPlayer): boolean {
    ...
    try {
      ...
      const promiseUpdateMediaList = new Promise((resolve, reject) => {
        ...
        const limitTotal = this.config.albumFavBrowserItemsLimit || this.LIMIT_TOTAL_MAX;
        const sortResult = this.config.albumFavBrowserItemsSortTitle || false;
        const market = null;
        ...
        this.spotifyPlusService.GetAlbumFavorites(player, 0, 0, market, limitTotal, sortResult)
          ...
          .catch(error => {
            ...
            super.updatedMediaListError("Get Album Favorites failed: " + getHomeAssistantErrorMessage(error));
      ...
      super.updatedMediaListError("Album favorites refresh failed: " + getHomeAssistantErrorMessage(error));
      return true;
