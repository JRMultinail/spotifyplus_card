  ...
  protected override updateMediaList(player: MediaPlayer): boolean {
    ...
    try {
      ...
      const promiseUpdateMediaList = new Promise((resolve, reject) => {
        ...
        this.spotifyPlusService.GetArtistsFollowed(player, 0, 0, limitTotal, sortResult)
          .then(result => {
          ...
          .catch(error => {
            ...
            super.updatedMediaListError("Get Artist Followed failed: " + getHomeAssistantErrorMessage(error));
      ...
      super.updatedMediaListError("Artist followed refresh failed: " + getHomeAssistantErrorMessage(error));
      return true;
